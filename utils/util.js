//const data = await fetch(`https://api.ocr.space/parse/imageurl`, {query: {apikey: 'K81569313788957', url, detectOrientation: true, scale: true, OCREngine: 3, ...language}})${data.ProcessingTimeInMilliseconds/1000} сек.
const hex2rgb = hex => {
...
const ruMs = time => {
	const obj = ms(time);
	const data = [
	obj.days !== 0 ? `${plur(['день', 'дня', 'дней'], obj.days)} ` : '',
	obj.hours !== 0 ? `${plur(['час', 'часа', 'часов'], obj.hours)} ` : '',
	obj.minutes !== 0 ? `${plur(['минута', 'минуты', 'минут'], obj.minutes)} ` : '',
	obj.seconds !== 0 ? `${plur(['секунда', 'секунды', 'секунд'], obj.seconds)}` : ''
	]
	return data.join('');
};
...
const snitchPantone = async() => {
	...
	require('fs').writeFile('colors.json', JSON.stringify({colors: data}), 'utf8', (err) => err ? console.error(err) : console.log('Пантон спизжен'));
	return true
};
module.exports = {
	colors: {
	...
	consts: {
		...
		replies: ['Защищаю пентагон...', 'Навожу ракеты на кремль...',
'Строю биолаборатории...', ' Запускаю «Нептун»...',
'Строю АТБ на красной площади...', 'Стреляю в Т-90М из джавелина...',
'Навожу ракеты на крымский мост...', 'Нахожу бункер путина...',
'Вырываю Украинское море...', 'Захватываю Кубань...',
'Перевожу с свинособачьего...', 'Воскрешаю Бандеру...',
'Забиваю окна в расею...', 'Делаю руззкий мёртвым зыком...',
'Разворачиваю ленд-лиз...', 'Запускаю Призрака Киева...',
'Заставляю орков сказать «Паляниця»...', 'Закупаю пакеты для груза 200...']
	}
};
